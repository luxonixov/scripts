...
Background.Position = UDim2.new(0, 400, 0, 160)
Background.Size = UDim2.new(0, 380, 0, 230)

LeftPanel.Name = "LeftPanel"
LeftPanel.Parent = Background
LeftPanel.BackgroundColor3 = Color3.fromRGB(53, 52, 55)
LeftPanel.BorderSizePixel = 0
LeftPanel.Position = UDim2.new(0, 0, 0, 16)
LeftPanel.Size = UDim2.new(0, 110, 0, 214)
...
LogList.Position = UDim2.new(0, 0, 0, 7)
LogList.Size = UDim2.new(0, 110, 0, 200)
...
RemoteTemplate.Size = UDim2.new(0, 100, 0, 24)
...
ColorBar.Size = UDim2.new(0, 5, 0, 16)
ColorBar.ZIndex = 2
...
RightPanel.Position = UDim2.new(0, 110, 0, 16)
RightPanel.Size = UDim2.new(0, 270, 0, 214)

CodeBox.Name = "CodeBox"
CodeBox.Parent = RightPanel
CodeBox.BackgroundColor3 = Color3.new(0.0823529, 0.0745098, 0.0784314)
CodeBox.BorderSizePixel = 0
CodeBox.Size = UDim2.new(0, 270, 0, 110)
...
ScrollingFrame.Size = UDim2.new(1, 0, 0.5, -8)
...
UIGridLayout.CellSize = UDim2.new(0, 84, 0, 24)
...
FunctionTemplate.Size = UDim2.new(0, 100, 0, 20)
...
ColorBar_2.Position = UDim2.new(0, 5, 0, 8)
ColorBar_2.Size = UDim2.new(0, 5, 0, 16)
ColorBar_2.ZIndex = 3
...
Text_2.Position = UDim2.new(0, 14, 0, 8)
Text_2.Size = UDim2.new(0, 62, 0, 16)
Text_2.ZIndex = 2
...
Simple.Size = UDim2.new(0, 57, 0, 16)
...
MaximizeButton.Position = UDim2.new(1, -32, 0, 0)
...
ImageLabel_2.Size = UDim2.new(0, 8, 0, 8)
...
ImageLabel_3.Position = UDim2.new(0, 4, 0, 4)
...
function toggleMinimize(override)
	...
	if closed then
		...
		TweenService:Create(LeftPanel, TweenInfo.new(0.5), { Size = UDim2.new(0, 110, 0, 0) }):Play()
		wait(0.5)
		remotesFadeIn = fadeOut(LeftPanel:GetDescendants())
		wait(0.5)
	else
		TweenService:Create(LeftPanel, TweenInfo.new(0.5), { Size = UDim2.new(0, 110, 0, 214) }):Play()
		wait(0.5)
	...
	if inResizeRange then
		...
		RunService:BindToRenderStep("SIMPLESPY_RESIZE", 1, function()
			...
			if newPos ~= lastPos then
				...
				if currentX < 380 then currentX = 380 end
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	id.Name = "ID"; id.Value = #logs + 1; id.Parent = remoteFrame
	local weakRemoteTable = setmetatable({ remote = remote }, { __mode = "v" })
	local log = {
		Name = name, Function = function_info, Remote = weakRemoteTable, Log = remoteFrame,
		Blocked = blocked, Source = src,
	...
	schedule(function()
		...
		if blocked then logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY SIMPLESPY\n\n" .. logs[#logs].GenScript end
	end)
...
function f2s(f)
	...
	if funcEnabled and debug.getinfo(f).name:match("^[%a_]+[%w_]*$") then return "function()end --[[" .. debug.getinfo(f).name .. "]]" end
...
function u2s(u)
	...
	elseif typeof(u) == "NumberSequence" then
		...
		for i, v in pairs(u.KeyPoints) do ret = ret .. tostring(v); if i < #u.Keypoints then ret = ret .. ", " end end
		return ret .. ")"
	elseif typeof(u) == "DockWidgetPluginGuiInfo" then return "DockWidgetPluginGuiInfo.new(Enum.InitialDockState" .. tostring(u) .. ")"
	elseif typeof(u) == "ColorSequence" then
		local ret = "ColorSequence.new("
		for i, v in pairs(u.KeyPoints) do ret = ret .. "Color3.new(" .. tostring(v) .. ")"; if i < #u.Keypoints then ret = ret .. ", " end end
		return ret .. ")"
	...
	elseif typeof(u) == "Region3" then
		local center = u.CFrame.Position; local size = u.CFrame.Size
	...
	elseif typeof(u) == "PathWaypoint" then return string.format("PathWaypoint.new(%s, %s)", v2s(u.Position), v2s(u.Action))
	...
	if realPath then
		...
		repeat
			...
			if not e then
				...
				local test = realPath.FindFirstChild(realPath, runningTest)
				local yeOld = e
				if test then realPath = test; s = e + 2; e = src:sub(e + 2, -1):find("%.")
					e = e and e + yeOld or e
				else e = src:sub(e + 2, -1):find("%.")
					e = e and e + yeOld or e end
			end
		until match or i >= 50
	end
	return realPath
...
function hookRemote(remoteType, remote, ...)
	if typeof(remote) == "Instance" then
		...
		if validInstance and not (blacklist[remote] or blacklist[remoteName]) then
			...
			if funcEnabled then funcInfo = debug.getinfo(4) or funcInfo; calling = useGetCallingScript and getcallingscript() or nil end
